pipeline {
  ...
  stages {
    stage('Deploy CloudHub') { 
      environment { 
	        CREDENTIAL_TEST_FR = credentials('credential-test-fr')
      ...
      steps {
	      script{
		      if(isUnix()){
        sh '''
		mvn deploy -P cloudhub -Dmule.version=4.2.2 -Danypoint.username=${CREDENTIAL_TEST_FR_USR} -Danypoint.password=${CREDENTIAL_TEST_FR_PSW} -Dcloudhub.application.name=flow-training-example
