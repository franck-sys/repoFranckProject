    ...
    stage('Deploy CloudHub') { 
      ...
      steps {
	      script{
		      if(isUnix()){
        sh '''
		mvn deploy -P cloudhub -Dmule.version=4.2.2 -Dusername=${CREDENTIAL_TEST_FR_USR} -Dpassword=${CREDENTIAL_TEST_FR_PWD} -Dcloudhub.application.name=flow-training-example
