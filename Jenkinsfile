    ...
    stage('Deploy CloudHub') { 
      ...
      steps {
	      script{
		      if(isUnix()){
        sh '''
		mvn deploy -P cloudhub -Dmule.version=4.2.2 -Danypoint.username=${ANYPOINT_CREDENTIALS_Username} -Danypoint.password=${ANYPOINT_CREDENTIALS_Password} -Dcloudhub.application.name=flow-training-example
