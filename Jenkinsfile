pipeline {
  ...
  stages {
    stage('Deploy CloudHub') { 
      environment { 
	        ANYPOINT_CREDENZIALS = credentials('credential-test-fr')
      ...
      steps {
	      script{
		      if(isUnix()){
        sh '''
		mvn deploy -P cloudhub -Dmule.version=4.2.2 -Danypoint.username=${ANYPOINT_CREDENZIALS_USR} -Danypoint.password=${ANYPOINT_CREDENZIALS_PSW} -Dcloudhub.application.name=flow-training-example
